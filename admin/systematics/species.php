...
$db = getDbInstance();
$cols = array(
    'sp.id id', 'sp.incertae_sedis incertae_sedis', 'sp.validate validate', 'sp.redirect redirect', 'sp.published published',
    'tx.name taxon'
);
...
if ($search_string)
{
    $db->where('sp.genus', '%'.$search_string.'%', 'like');
    $db->orWhere('sp.species', '%'.$search_string.'%', 'like');
    $db->orWhere('tx.name', '%'.$search_string.'%', 'like');
}
    ...
    <div class="toolbar sticky-top row my-2 p-2">
        <div class="col-12">
            ...
            <div class="float-right">
                <a href="specie.php?task=new&id=0" class="btn btn-primary btn-sm" role="button"><i class="fas fa-plus"></i>New</a>
            </div>
        </div>
    </div>
    <!-- Main -->
        ...
        <div class="col-12">
            <div class="my-3 p-3 bg-white rounded box-shadow">
                ...
                <form class="form-inline mb-3" action="">
                    <label>
                        Search
                        <span data-toggle="tooltip" data-placement="top" title="Genus, Species or Taxa">
                    ...
                    <select name="order_by" class="form-control mr-2">
                        <?php
                        foreach ($species->getOrderValues() as $opt_value => $opt_name):
                ...
                <table class="table table-striped table-hover table-sm">
                    ...
                    <thead>
                        <tr width="100%">
                            <th width="5%"><a href="<?php echo $file_name; ?>.php?order_by=sp.id">ID</a></th>
                            <th width="45%"><a href="<?php echo $file_name; ?>.php?order_by=sp.genus">Species</a></th>
                            <th width="22.5%"><a href="<?php echo $file_name; ?>.php?order_by=tx.name">Taxon</a></th>
                            <th width="22.5%"><a href="<?php echo $file_name; ?>.php?order_by=sp.validate">Validate</a></th>
                            <th width="5%" colspan="2"><a href="<?php echo $file_name; ?>.php?order_by=sp.published">State</a></th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        // Validade
                        if (!$row['validate']):
                            $redirect = (!$row['redirect']) ? '' : ' of '.$species->getNomenclature($row['redirect']);
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="specie.php?task=edit&id=<?php echo $row['id']; ?>"><?php echo $species->getNomenclature($row['id']); ?></a>
                        ...
                        <div class="modal fade" id="delete-<?php echo $row['id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <form action="specie_delete.php" method="POST">
